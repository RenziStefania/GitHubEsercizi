document.addEventListener("DOMContentLoaded", () => {
  ...
  async function fetchActivities() {
    try {
      ...
      // Clear loading message and current data
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantsList = participants.length
          ? `<ul class="participant-list">${participants
              .map(
                (participant) =>
                  `<li class="participant-item"><span>${participant}</span><button class="unregister-btn" data-activity="${name}" data-email="${participant}" title="Remove participant">✕</button></li>`
              )
              .join("")}</ul>`
  ...
  signupForm.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Refresh activity cards after sign-up success
